using System;
using System.Collections.Generic;
using System.Linq;
using Leopotam.EcsLite;

namespace PavEcsSpec.EcsLite
{
    //TODO: autogenerated this file
    public interface IEcsReadonlySpec : IEcsSpec
    {
    }
    public readonly struct EcsReadonlySpec : IEcsReadonlySpec, 
        IHasBuilder<EcsReadonlySpec>
    {
        public bool IsBelongToWorld(EcsWorld world)
        {
            return true;
        }

        public static Builder Empty() => new Builder();

        public IEcsSpecBuilder<EcsReadonlySpec> GetBuilder() => new Builder();

        public struct Builder : IEcsSpecBuilder<EcsReadonlySpec>
        {
            public EcsFilter.Mask Include(EcsWorld world)
            {
                throw new InvalidOperationException("Empty spec is impossible to use as include filter");
            }
            public EcsFilter.Mask Include(EcsFilter.Mask mask)
            {
                throw new InvalidOperationException("Empty spec is impossible to use as include filter");
            }

            public EcsFilter.Mask Exclude(EcsFilter.Mask mask)
            {
                return mask;
            }

            public IEnumerable<(Type type, SpecPermissions permission)> GetArgTypes()
            {
                return Enumerable.Empty<(Type type, SpecPermissions permission)>();
            }

            public EcsWorld GetWorld(EcsUniverse universe, EcsSystems systems)
            {
                return null;
            }

            public EcsReadonlySpec Create(EcsWorld world)
            {
                return new EcsReadonlySpec();
            }
        }

    }

    public readonly struct EcsReadonlySpec<T1> : IEcsReadonlySpec, IHasBuilder<EcsReadonlySpec<T1>>
        where T1 : struct
    {
        private readonly EcsWorld _world;

        public bool IsBelongToWorld(EcsWorld world)
        {
            return _world == world;
        }

        public EcsReadonlyPool<T1> Pool1 { get; }

        internal EcsReadonlySpec(EcsWorld world)
        {
            _world = world;
            Pool1 = world.GetPool<T1>();
        }

        public void Deconstruct(out EcsReadonlyPool<T1> pool1)
        {
            pool1 = Pool1;
        }

        public static implicit operator EcsReadonlyPool<T1>(in EcsReadonlySpec<T1> spec)
        {
            return spec.Pool1;
        }

        public static Builder Build() => Builder.Instance;
        public IEcsSpecBuilder<EcsReadonlySpec<T1>> GetBuilder() => Builder.Instance;


        public class Builder : IEcsSpecBuilder<EcsReadonlySpec<T1>>
        {
            internal static Builder Instance { get; } = new Builder();
            public EcsFilter.Mask Include(EcsWorld world)
            {
                return world.Filter<T1>();
            }

            public EcsFilter.Mask Include(EcsFilter.Mask mask)
            {
                return mask
                    .Inc<T1>();
            }

            public EcsFilter.Mask Exclude(EcsFilter.Mask mask)
            {
                return mask.Exc<T1>();
            }

            public EcsReadonlySpec<T1> Create(EcsWorld world)
            {
                return new EcsReadonlySpec<T1>(world);
            }

            public IEnumerable<(Type type, SpecPermissions permission)> GetArgTypes()
            {
                yield return (typeof(T1), SpecPermissions.Read);
            }

            public EcsWorld GetWorld(EcsUniverse universe, EcsSystems systems)
            {
                return universe.GetWorld<T1>(systems);
            }
            public SpecPermissions GetPermissions() => SpecPermissions.Read;

        }
    }

    public readonly struct EcsReadonlySpec<T1, T2> : IEcsReadonlySpec, IHasBuilder<EcsReadonlySpec<T1,T2>>
        where T1 : struct
        where T2 : struct
    {
        public EcsReadonlyPool<T1> Pool1 { get; }

        public EcsReadonlyPool<T2> Pool2 { get; }

        private readonly EcsWorld _world;

        public bool IsBelongToWorld(EcsWorld world)
        {
            return _world == world;
        }
        //internal EcsReadonlySpec(in EcsReadonlySpec<T1> ecsSpec, EcsReadonlyPool<T2> pool2)
        //{
        //    Pool1 = ecsSpec.Pool1;
        //    Pool2 = pool2;
        //    _world = ecsSpec._world;
        //}

        internal EcsReadonlySpec(
            EcsWorld world)
        {
            _world = world;
            Pool1 = world.GetPool<T1>();
            Pool2 = world.GetPool<T2>();
        }

        public void Deconstruct(
            out EcsReadonlyPool<T1> pool1,
            out EcsReadonlyPool<T2> pool2
        )
        {
            pool1 = Pool1;
            pool2 = Pool2;
        }

        public static Builder Build() => Builder.Instance;
        public IEcsSpecBuilder<EcsReadonlySpec<T1, T2>> GetBuilder() => Builder.Instance;

        public class Builder : IEcsSpecBuilder<EcsReadonlySpec<T1, T2>>
        {
            internal static Builder Instance { get; } = new Builder();

            public EcsFilter.Mask Include(EcsWorld world)
            {
                return world
                    .Filter<T1>()
                    .Inc<T2>();
            }

            public EcsFilter.Mask Include(EcsFilter.Mask mask)
            {
                return mask
                    .Inc<T1>()
                    .Inc<T2>();
            }

            public EcsFilter.Mask Exclude(EcsFilter.Mask mask)
            {
                return mask
                    .Exc<T1>()
                    .Exc<T2>();
            }

            public EcsReadonlySpec<T1, T2> Create(EcsWorld world)
            {
                return new EcsReadonlySpec<T1, T2>(world);
            }

            public IEnumerable<(Type type, SpecPermissions permission)> GetArgTypes()
            {
                yield return (typeof(T1), SpecPermissions.Read);
                yield return (typeof(T2), SpecPermissions.Read);
            }

            public EcsWorld GetWorld(EcsUniverse universe, EcsSystems systems)
            {
                return universe.GetWorld<T1>(systems);
            }
        }
    }

    public readonly struct EcsReadonlySpec<T1, T2, T3> : IEcsReadonlySpec, IHasBuilder<EcsReadonlySpec<T1,T2,T3>>
        where T1 : struct
        where T2 : struct
        where T3 : struct
    {
        public EcsReadonlyPool<T1> Pool1 { get; }

        public EcsReadonlyPool<T2> Pool2 { get; }

        public EcsReadonlyPool<T3> Pool3 { get; }

        private readonly EcsWorld _world;

        public bool IsBelongToWorld(EcsWorld world)
        {
            return _world == world;
        }

        internal EcsReadonlySpec(EcsWorld world)
        {
            _world = world;
            Pool1 = world.GetPool<T1>();
            Pool2 = world.GetPool<T2>();
            Pool3 = world.GetPool<T3>();
        }

        public void Deconstruct(
            out EcsReadonlyPool<T1> pool1,
            out EcsReadonlyPool<T2> pool2,
            out EcsReadonlyPool<T3> pool3
        )
        {
            pool1 = Pool1;
            pool2 = Pool2;
            pool3 = Pool3;
        }

        public static Builder Build() => Builder.Instance;
        public IEcsSpecBuilder<EcsReadonlySpec<T1, T2, T3>> GetBuilder() => Builder.Instance;

        public class Builder : IEcsSpecBuilder<EcsReadonlySpec<T1, T2, T3>>
        {
            internal static Builder Instance { get; } = new Builder();

            public EcsFilter.Mask Include(EcsWorld world)
            {
                return world
                    .Filter<T1>()
                    .Inc<T2>()
                    .Inc<T3>();
            }

            public EcsFilter.Mask Include(EcsFilter.Mask mask)
            {
                return mask
                    .Inc<T1>()
                    .Inc<T2>()
                    .Inc<T3>();
            }

            public EcsFilter.Mask Exclude(EcsFilter.Mask mask)
            {
                return mask
                    .Exc<T1>()
                    .Exc<T2>()
                    .Exc<T3>();
            }

            public EcsReadonlySpec<T1, T2, T3> Create(EcsWorld world)
            {
                return new EcsReadonlySpec<T1, T2, T3>(world);
            }
            public IEnumerable<(Type type, SpecPermissions permission)> GetArgTypes()
            {
                yield return (typeof(T1), SpecPermissions.Read);
                yield return (typeof(T2), SpecPermissions.Read);
                yield return (typeof(T3), SpecPermissions.Read);
            }

            public EcsWorld GetWorld(EcsUniverse universe, EcsSystems systems)
            {
                return universe.GetWorld<T1>(systems);
            }
        }
    }

    public readonly struct EcsReadonlySpec<T1, T2, T3, T4> : IEcsReadonlySpec, IHasBuilder<EcsReadonlySpec<T1,T2,T3,T4>>
        where T1 : struct
        where T2 : struct
        where T3 : struct
        where T4 : struct
    {
        public EcsReadonlyPool<T1> Pool1 { get; }

        public EcsReadonlyPool<T2> Pool2 { get; }

        public EcsReadonlyPool<T3> Pool3 { get; }

        public EcsReadonlyPool<T4> Pool4 { get; }

        private readonly EcsWorld _world;

        public bool IsBelongToWorld(EcsWorld world)
        {
            return _world == world;
        }

        internal EcsReadonlySpec(EcsWorld world)
        {
            _world = world;
            Pool1 = world.GetPool<T1>();
            Pool2 = world.GetPool<T2>();
            Pool3 = world.GetPool<T3>();
            Pool4 = world.GetPool<T4>();
        }

        public void Deconstruct(
            out EcsReadonlyPool<T1> pool1,
            out EcsReadonlyPool<T2> pool2,
            out EcsReadonlyPool<T3> pool3,
            out EcsReadonlyPool<T4> pool4
        )
        {
            pool1 = Pool1;
            pool2 = Pool2;
            pool3 = Pool3;
            pool4 = Pool4;
        }

        public static Builder Build() => Builder.Instance;
        public IEcsSpecBuilder<EcsReadonlySpec<T1, T2, T3, T4>> GetBuilder() => Builder.Instance;

        public class Builder : IEcsSpecBuilder<EcsReadonlySpec<T1, T2, T3, T4>>
        {
            internal static Builder Instance { get; } = new Builder();

            public EcsFilter.Mask Include(EcsWorld world)
            {
                return world
                    .Filter<T1>()
                    .Inc<T2>()
                    .Inc<T3>()
                    .Inc<T4>();
            }

            public EcsFilter.Mask Include(EcsFilter.Mask mask)
            {
                return mask
                    .Inc<T1>()
                    .Inc<T2>()
                    .Inc<T3>()
                    .Inc<T4>();
            }

            public EcsFilter.Mask Exclude(EcsFilter.Mask mask)
            {
                return mask
                    .Exc<T1>()
                    .Exc<T2>()
                    .Exc<T3>()
                    .Exc<T4>();
            }

            public EcsReadonlySpec<T1, T2, T3, T4> Create(EcsWorld world)
            {
                return new EcsReadonlySpec<T1, T2, T3, T4>(world);
            }

            public IEnumerable<(Type type, SpecPermissions permission)> GetArgTypes()
            {
                yield return (typeof(T1), SpecPermissions.Read);
                yield return (typeof(T2), SpecPermissions.Read);
                yield return (typeof(T3), SpecPermissions.Read);
                yield return (typeof(T4), SpecPermissions.Read);
            }

            public EcsWorld GetWorld(EcsUniverse universe, EcsSystems systems)
            {
                return universe.GetWorld<T1>(systems);
            }
        }
    }

    public readonly struct EcsReadonlySpec<T1, T2, T3, T4, T5> : IEcsReadonlySpec, IHasBuilder<EcsReadonlySpec<T1,T2,T3,T4,T5>>
        where T1 : struct
        where T2 : struct
        where T3 : struct
        where T4 : struct
        where T5 : struct
    {
        public EcsReadonlyPool<T1> Pool1 { get; }

        public EcsReadonlyPool<T2> Pool2 { get; }

        public EcsReadonlyPool<T3> Pool3 { get; }

        public EcsReadonlyPool<T4> Pool4 { get; }

        public EcsReadonlyPool<T5> Pool5 { get; }

        private readonly EcsWorld _world;

        public bool IsBelongToWorld(EcsWorld world)
        {
            return _world == world;
        }

        internal EcsReadonlySpec(EcsWorld world)
        {
            _world = world;
            Pool1 = world.GetPool<T1>();
            Pool2 = world.GetPool<T2>();
            Pool3 = world.GetPool<T3>();
            Pool4 = world.GetPool<T4>();
            Pool5 = world.GetPool<T5>();
        }

        public void Deconstruct(
            out EcsReadonlyPool<T1> pool1,
            out EcsReadonlyPool<T2> pool2,
            out EcsReadonlyPool<T3> pool3,
            out EcsReadonlyPool<T4> pool4,
            out EcsReadonlyPool<T5> pool5
        )
        {
            pool1 = Pool1;
            pool2 = Pool2;
            pool3 = Pool3;
            pool4 = Pool4;
            pool5 = Pool5;
        }

        public static Builder Build() =>  Builder.Instance;
        public IEcsSpecBuilder<EcsReadonlySpec<T1, T2, T3, T4, T5>> GetBuilder() => Builder.Instance;

        public class Builder : IEcsSpecBuilder<EcsReadonlySpec<T1, T2, T3, T4, T5>>
        {
            internal static Builder Instance { get; } = new Builder();

            public EcsFilter.Mask Include(EcsWorld world)
            {
                return world
                    .Filter<T1>()
                    .Inc<T2>()
                    .Inc<T3>()
                    .Inc<T4>()
                    .Inc<T5>();
            }

            public EcsFilter.Mask Include(EcsFilter.Mask mask)
            {
                return mask
                    .Inc<T1>()
                    .Inc<T2>()
                    .Inc<T3>()
                    .Inc<T4>()
                    .Inc<T5>();
            }

            public EcsFilter.Mask Exclude(EcsFilter.Mask mask)
            {
                return mask
                    .Exc<T1>()
                    .Exc<T2>()
                    .Exc<T3>()
                    .Exc<T4>()
                    .Exc<T5>();
            }

            public EcsReadonlySpec<T1, T2, T3, T4, T5> Create(EcsWorld world)
            {
                return new EcsReadonlySpec<T1, T2, T3, T4, T5>(world);
            }

            public IEnumerable<(Type type, SpecPermissions permission)> GetArgTypes()
            {
                yield return (typeof(T1), SpecPermissions.Read);
                yield return (typeof(T2), SpecPermissions.Read);
                yield return (typeof(T3), SpecPermissions.Read);
                yield return (typeof(T4), SpecPermissions.Read);
                yield return (typeof(T5), SpecPermissions.Read);
            }

            public EcsWorld GetWorld(EcsUniverse universe, EcsSystems systems)
            {
                return universe.GetWorld<T1>(systems);
            }
        }
    }

    public readonly struct EcsReadonlySpec<T1, T2, T3, T4, T5, T6> : IEcsReadonlySpec, IHasBuilder<EcsReadonlySpec<T1,T2,T3,T4,T5,T6>>
        where T1 : struct
        where T2 : struct
        where T3 : struct
        where T4 : struct
        where T5 : struct
        where T6 : struct
    {
        public EcsReadonlyPool<T1> Pool1 { get; }

        public EcsReadonlyPool<T2> Pool2 { get; }

        public EcsReadonlyPool<T3> Pool3 { get; }

        public EcsReadonlyPool<T4> Pool4 { get; }

        public EcsReadonlyPool<T5> Pool5 { get; }

        public EcsReadonlyPool<T6> Pool6 { get; }

        private readonly EcsWorld _world;

        public bool IsBelongToWorld(EcsWorld world)
        {
            return _world == world;
        }

        internal EcsReadonlySpec(EcsWorld world)
        {
            _world = world;
            Pool1 = world.GetPool<T1>();
            Pool2 = world.GetPool<T2>();
            Pool3 = world.GetPool<T3>();
            Pool4 = world.GetPool<T4>();
            Pool5 = world.GetPool<T5>();
            Pool6 = world.GetPool<T6>();
        }

        public void Deconstruct(
            out EcsReadonlyPool<T1> pool1,
            out EcsReadonlyPool<T2> pool2,
            out EcsReadonlyPool<T3> pool3,
            out EcsReadonlyPool<T4> pool4,
            out EcsReadonlyPool<T5> pool5,
            out EcsReadonlyPool<T6> pool6
        )
        {
            pool1 = Pool1;
            pool2 = Pool2;
            pool3 = Pool3;
            pool4 = Pool4;
            pool5 = Pool5;
            pool6 = Pool6;
        }

        public static Builder Build() => Builder.Instance;
        public IEcsSpecBuilder<EcsReadonlySpec<T1, T2, T3, T4, T5, T6>> GetBuilder() => Builder.Instance;


        public class Builder : IEcsSpecBuilder<EcsReadonlySpec<T1, T2, T3, T4, T5, T6>>
        {
            internal static Builder Instance { get; } = new Builder();

            public EcsFilter.Mask Include(EcsWorld world)
            {
                return world
                    .Filter<T1>()
                    .Inc<T2>()
                    .Inc<T3>()
                    .Inc<T4>()
                    .Inc<T5>()
                    .Inc<T6>();
            }
            public EcsFilter.Mask Include(EcsFilter.Mask mask)
            {
                return mask
                    .Inc<T1>()
                    .Inc<T2>()
                    .Inc<T3>()
                    .Inc<T4>()
                    .Inc<T5>()
                    .Inc<T6>();
            }


            public EcsFilter.Mask Exclude(EcsFilter.Mask mask)
            {
                return mask
                    .Exc<T1>()
                    .Exc<T2>()
                    .Exc<T3>()
                    .Exc<T4>()
                    .Exc<T5>()
                    .Exc<T6>();
            }

            public EcsReadonlySpec<T1, T2, T3, T4, T5, T6> Create(EcsWorld world)
            {
                return new EcsReadonlySpec<T1, T2, T3, T4, T5, T6>(world);
            }

            public IEnumerable<(Type type, SpecPermissions permission)> GetArgTypes()
            {
                yield return (typeof(T1), SpecPermissions.Read);
                yield return (typeof(T2), SpecPermissions.Read);
                yield return (typeof(T3), SpecPermissions.Read);
                yield return (typeof(T4), SpecPermissions.Read);
                yield return (typeof(T5), SpecPermissions.Read);
                yield return (typeof(T6), SpecPermissions.Read);
            }

            public EcsWorld GetWorld(EcsUniverse universe, EcsSystems systems)
            {
                return universe.GetWorld<T1>(systems);
            }
        }
    }
}