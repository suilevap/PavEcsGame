...
namespace Tetris
{
    internal class FiguresData
    {
        public static FiguresData Instance { get; } = new FiguresData();
        ...
        public FigureType GetRandomFigureType(Random rnd) =>
            (FigureType)rnd.Next((int) FiguresData.FigureType.Max + 1);

        public uint GetFigureData(FigureType type, byte rotation)
        {
            var data = _shapesRotation[(int) type];
            return (uint)data[rotation % data.Length];
        }

        public uint GetFigureData(in Figure fig) => GetFigureData(fig.Shape, fig.Rotation);
    }
}
